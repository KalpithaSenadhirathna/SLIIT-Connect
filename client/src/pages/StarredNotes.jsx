...
const STARRED_NOTES = [
  { id:1, color:"blue",   subject:"💻 IT / SE",   subjectKey:"it",   title:"Data Structures & Algorithms — Week 5 Summary",     excerpt:"Big-O notation, BSTs, AVL rotations and heap operations. Includes solved past paper questions and a time complexity cheat sheet.", tags:["DSA","Trees","Algorithms","Y2S1"], author:"Asel Kumara",  av:"AK", avColor:"#4f6ef7", date:"Mar 18, 2026", likes:24, saves:12, liked:true,  saved:true,  pinned:true  },
  ...
  { id:3, color:"purple", subject:"🌐 Networking", subjectKey:"net",  title:"Network Security — Cryptography & Firewalls",        excerpt:"RSA key exchange, TLS handshake, firewall rule design, MITM/DDoS/SQL injection attack types. Exam-ready summaries.",               tags:["Security","RSA","Firewalls"],    author:"Malith R.",     av:"MR", avColor:"#7c3aed", date:"Feb 24, 2026", likes:29, saves:16, liked:false, saved:true,  pinned:true  },
];

const colorBar = { blue:"linear-gradient(90deg,#4f6ef7,#818cf8)", yellow:"linear-gradient(90deg,#f59e0b,#fbbf24)", purple:"linear-gradient(90deg,#8b5cf6,#a78bfa)" };
const subjectStyle = { it:{bg:"#eef1ff",color:"#4f6ef7"}, db:{bg:"#fffbeb",color:"#d97706"}, net:{bg:"#f5f3ff",color:"#7c3aed"} };
...
function NoteCard({ note, index, onLike, onSave, onPin }) {
  ...
  const ss = subjectStyle[note.subjectKey] || {bg:"#eef1ff",color:"#4f6ef7"};
  return (
    <div ref={ref} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
      style={{ background:"#fff", border:`1px solid ${hovered?"#dde0ea":"#e8eaf0"}`, borderRadius:14, overflow:"hidden", display:"flex", flexDirection:"column", cursor:"pointer", boxShadow:hovered?"0 8px 30px rgba(0,0,0,0.10)":"0 1px 4px rgba(0,0,0,0.04)", transform:visible?(hovered?"translateY(-4px)":"translateY(0)"):"translateY(16px)", opacity:visible?1:0, transition:"opacity 0.45s ease, transform 0.35s ease" }}>
      <div style={{ height:4, background:colorBar[note.color]||"linear-gradient(90deg,#4f6ef7,#818cf8)", flexShrink:0 }} />
      ...
      <div style={{ borderTop:"1px solid #e8eaf0", padding:"10px 17px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <div><div style={{ fontSize:12, color:"#4a5068", fontWeight:500 }}>{note.author}</div><div style={{ fontSize:11, color:"#b0b5c8" }}>{note.date}</div></div>
        </div>
...
export default function StarredNotes() {
  ...
  const [notes, setNotes] = useState(STARRED_NOTES);
  ...
  return (
    <div style={{ fontFamily:"'Plus Jakarta Sans',sans-serif", background:"#f0f2f8", minHeight:"calc(100vh - 100px)" }}>
      ...
      <div className="notes-sub-nav">
          ...
          <button className="sub-nav-btn active" onClick={() => navigate('/starred-notes')}>Starred</button>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column" }}>
        <div style={{ flex:1, overflowY:"auto", padding:"0 0 40px" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <div>
                <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:10, fontSize:13, color:"#8a91a8" }}>
                  ...
                  <span style={{ color:"#f59e0b", fontWeight:600 }}>Starred</span>
                </div>
                <div style={{ height:3, width:36, borderRadius:2, background:"linear-gradient(90deg,#f59e0b,#fbbf24)", marginBottom:10 }} />
                <h1 style={{ fontSize:20, fontWeight:800, color:"#1a1c2e", letterSpacing:"-0.4px", lineHeight:1.2, marginBottom:3 }}>Starred Notes</h1>
                <p style={{ fontSize:13, color:"#8a91a8" }}>Notes you've pinned and starred for quick access.</p>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:8, background:"#fff", border:"1px solid #dde0ea", borderRadius:12, padding:"10px 18px", width:240, boxShadow:"0 2px 8px rgba(0,0,0,0.06)" }}>
                ...
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search starred notes…" style={{ background:"none", border:"none", outline:"none", fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:13, color:"#1a1c2e", width:"100%" }} />
              </div>
            </div>
          </div>

          <div style={{ padding:"0 36px", display:"flex", gap:12, marginBottom:24 }}>
            {[{icon:"⭐",bg:"#fffbeb",value:`${STARRED_NOTES.length} Starred`,label:"Notes pinned"},{icon:"📚",bg:"#eef1ff",value:"3 Subjects",label:"Across subjects"},{icon:"🔖",bg:"#ecfdf5",value:"47 Saves",label:"Total saves"}].map((s,i)=>(
          ...
          <div style={{ padding:"0 32px" }}>
            {filtered.length===0
              ? <div style={{ textAlign:"center", padding:"80px 0", color:"#8a91a8" }}><div style={{ fontSize:40, marginBottom:14 }}>⭐</div><div style={{ fontSize:15, fontWeight:600 }}>No starred notes found</div></div>
