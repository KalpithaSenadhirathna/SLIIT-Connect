...
const handleAttendanceSubmit = async (e) => {
  ...
  const currentRecord = filteredData[0];
  try {
    await axios.put(`http://localhost:5000/api/sessions/addAttendance/${currentRecord._id}`, {
      stuId: inputs.stuId,
    ...
    /*setFilteredData(prev =>
      prev.map(item =>
        item._id === currentRecord._id
          ? { ...item, stuId: inputs.stuId,stuName: inputs.stuName, status: inputs.status }
          : item
      )
    );*/
    setFilteredData(prev =>
  prev.map(item =>
    ...
    setInputs(prev => ({ ...prev, stuId: "",stuName: "", status: "" }));
    alert("sumitted successfully!");
  } catch (error) {
    console.error("Error occurred while adding dat:", error);
...
const handleSelectToggle = (id) => {
  ...
  if (selectedSessions.includes(idStr)) {
    updated = selectedSessions.filter(item => item !== idStr); 
  } else {
    updated = [...selectedSessions, idStr]; 
  }

  setSelectedSessions(updated);
  localStorage.setItem("selectedSessions", JSON.stringify(updated));
};
  ...
  />
                                ...
                                <div className="card-content"/*style={containerStyle1}*/>
                                    ...
                                    <div style={cardStyle}>
                                    <h1 className="card-title"><b>name:</b> {data.Name}</h1></div><br></br>
                                    <div style={cardStyle}>
                                    <h1 className="card-description">Description: {data.Description}</h1></div><br></br>
                                    <div style={cardStyle}>
                                    <h1 className="card-description">Year: {data.Year}</h1></div><br></br>
                                    <div style={cardStyle}>
                                    <h1 className="card-description">Semester: {data.Semester}</h1></div><br></br>
                                    <div style={cardStyle}>
                                    <h1 className="card-description">Module: {data.Module}</h1></div><br></br>
                                    <div style={cardStyle}>
                                    ...
                                    <div style={cardStyle}>
                                    <h1 className="card-description">Date: {data.Date}</h1></div><br></br>
                                    <br></br>
                                    <div style={cardStyle}>
                            ...
                            style={{
                            ...buttonStyle,
                            backgroundColor: selectedSessions.includes(data._id)
                             ? "green"
                            : "#100c35ff"
                                }}
                            onClick={() => handleSelectToggle(data._id)}
                            >
                                {selectedSessions.includes(data._id) ? "UnSelect" : "Select"}
                            </button>
        ...
        {attendanceSubmitted ? (
  <div style={{ ...formStyle, textAlign: "center", fontSize: "22px" }}>
    You have submitted your attendance.
  </div>
) : (

         <form style={formStyle} >
          <h1><b>Confirm your participation</b></h1><br></br>
  <p><h2>Enter student ID</h2><br></br></p>
   <label>
          <input type="text" name="stuId" style={inputStyle} value={inputs.stuId || "" }/* checked={inputs.Reminder === "weekly"} */ onChange={handleChange}  />
          
          </label><br></br>
    <p><h2>Enter student Name</h2><br></br></p>
   <label>
          <input type="text" name="stuName" style={inputStyle} value={inputs.stuName || "" }/* checked={inputs.Reminder === "weekly"} */ onChange={handleChange}  />
          
          </label><br></br>
           <label><h2> Particiating to the session?</h2><br></br></label>
            <label>
          <input type="radio" name="status" value="Participating" checked={inputs.status === "Participating"}  onChange={handleChange}  />
          Yes
          </label><br></br>
          <label>
          <input type="radio" name="status" value="Not participating"  checked={inputs.status === "Not participating"}  onChange={handleChange}/>
          No
          </label><br></br>
          <label>
          <input type="radio" name="status" value="May be"  checked={inputs.status === "May be"}  onChange={handleChange}/>
          May be
          </label>
           <br></br>
  <br />
